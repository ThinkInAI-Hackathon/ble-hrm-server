...
from qiniu import Auth as QiniuAuth, put_file as QiniuPutFile, etag as QiniuEtag
...
value is a dict of device name and rssi."""
    ...
    async def background_monitor(self, duration: int):
        ...
        async with self.client:
            ...
            await asyncio.sleep(duration)  # Keep listening for duration seconds
    ...
    def build_heart_rate_chart(self, since_from: float = 600.0) -> str:
        ...
        if bucket_len > 60:
            ...
            bucket_size = 1
        data = self.get_heart_rate_bucket(since_from=since_from, bucket_size=bucket_size)
        if not data:
        ...
        plt.axhline(y=avg_hr, color="r", linestyle="--", label=f"Average HR: {avg_hr:.1f} bpm")
        plt.xlabel("Time")
