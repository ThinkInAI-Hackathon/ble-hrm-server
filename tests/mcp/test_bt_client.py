...
async def test_list_bluetooth_devices(bt_client):
    ...
    with patch('mcp.bt_client.BleakScanner.discover', new=AsyncMock(return_value=devices)):
...
def test_build_heart_rate_chart(mock_plt, mock_upload_file, bt_client):
    ...
    with patch.object(bt_client, 'get_heart_rate_bucket', return_value=[{'time': 1, 'value': 60}, {'time': 2, 'value': 70}]):
